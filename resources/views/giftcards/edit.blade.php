    ...
    <div class="w-full max-w-xl bg-[#050f1b] text-a4cadc rounded-2xl shadow-lg p-8">
        ...
        <form action="{{ route('giftcards.update', $giftcard->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $giftcard->title) }}" required class="w-full p-2 rounded bg-[#142234] border border-gray-600 text-a4cadc" />
            </div>

            <div>
                <label for="description" class="block mb-1">Descripción</label>
                <textarea name="description" id="description" rows="3" class="w-full p-2 rounded bg-[#142234] border border-gray-600 text-a4cadc">{{ old('description', $giftcard->description) }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="amount" class="block mb-1">Monto</label>
                    <input type="number" name="amount" id="amount" value="{{ old('amount', $giftcard->amount) }}" required class="w-full p-2 rounded bg-[#142234] border border-gray-600 text-a4cadc" />
                </div>

                <div>
                    <label for="price" class="block mb-1">Precio</label>
                    <input type="number" name="price" id="price" step="0.01" value="{{ old('price', $giftcard->price) }}" required class="w-full p-2 rounded bg-[#142234] border border-gray-600 text-a4cadc" />
                </div>
            </div>
            ...
            <div>
                ...
                <input type="number" name="stock" id="stock" value="{{ old('stock', $giftcard->stock) }}" required class="w-full p-2 rounded bg-[#142234] border border-gray-600 text-a4cadc" />
            </div>
